<script lang="ts">
	...
	import {timelineSelection, calculateDeltaDays, dateToSliderDisplayIndex, sliderDisplayIndexToDate, sliderDisplayOffset} from '../state/timelineSelection.svelte'
	...
	const {
		elements: { root, range, thumbs, ticks }, options: {max}
	} = createSlider({
        value: sliderStore,
		min: 1,
...
<div use:melt={$root} class="relative flex h-16 w-full overflow-none">
	<span class="h-[3px] w-full self-start bg-neutral-6">
		<span use:melt={$range} class="h-[3px] bg-neutral-9" ></span>
	</span>

	<span use:melt={$thumbs[0]} class="h-1/3 w-1 rounded-md self-start bg-neutral-9" ></span>
	<span use:melt={$thumbs[1]} class="h-1/2 w-1 rounded-b-md self-start bg-primary-8" ></span>
	<span use:melt={$thumbs[2]} class="h-1/3 w-1 rounded-md self-start bg-neutral-9" ></span>

	{#each $ticks as tick, index}
        <span use:melt={tick} class="h-3 w-[3px] self-end rounded-t-lg bg-neutral-11"> </span>
	{/each}
</div>
