...
from src.verdantech_api.application.user.services.email.addition import (
    add_first_email,
...
async def test_add_new_email(
    ...
    mock_add_first_email_verification_required = mocker.patch(
        "src.verdantech_api.application.user.services.email.addition.EmailAdditionService.add_first_email_verification_required",
        return_value="abc",
    )
    mock_add_first_email_verification_not_required = mocker.patch(
        "src.verdantech_api.application.user.services.email.addition.EmailAdditionService.add_first_email_verification_not_required"
    )
    mock_emit_email_verification_email = mocker.patch(
        "src.verdantech_api.application.user.services.email.addition.emit_email_verification_email"
    )
...
async def test_new_email_verification(mocker: MockerFixture) -> None:
    ...
    mock_new_verification = mocker.patch(
        "src.verdantech_api.application.user.services.email.addition.EmailVerificationService.new_verification",
