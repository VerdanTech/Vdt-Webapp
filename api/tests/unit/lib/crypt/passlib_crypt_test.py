...
class TestPasslibCrypt:
    async def test_get_password_hash(self, passlib_password_crypt) -> None:
        ...
        secret_str_hash = await passlib_password_crypt.get_password_hash(secret_str)
        secret_bytes_hash = await passlib_password_crypt.get_password_hash(secret_bytes)
