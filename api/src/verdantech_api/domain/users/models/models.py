...
from src.verdantech_api.settings import (
    EMAIL_MAX_LENGTH,
...
class EmailModel(BigIntAuditBase):
    ...
    address: Mapped[str] = mapped_column(String(length=EMAIL_MAX_LENGTH), unique=True)
...
class VerificationModelMixin:
    """Attributes required for storing email confirmation information"""
...
class EmailConfirmationModel(BigIntAuditBase, VerificationModelMixin):
...
class PasswordResetConfirmationModel(BigIntAuditBase, VerificationModelMixin):
