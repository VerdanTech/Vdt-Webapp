...
from src.application.user.services.password.reset import (
...
async def test_new_password_reset(mocker: MockerFixture) -> None:
    ...
    mock_new_password_reset = mocker.patch(
        "src.application.user.services.password.reset.PasswordResetService.new_password_reset",
        return_value=generated_key,
    )
    mock_emit_password_reset_email = mocker.patch(
        "src.application.user.services.password.reset.emit_password_reset_email"
    )
