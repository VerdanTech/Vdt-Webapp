...
- For a type with a matching Observe type (Seed, Germinate, Transplant, Harvest, Expire - see [Actions models](models.md#completed)), completing it writes a minimal observation onto its Plant with today's date and no further detail, the same as Observe would. The row can be expanded inline to fill in the same fields Observe shows for that type (e.g. mass for a Harvest), without leaving the Workbook.
- For a type with no matching Observe type (Thin, Harden, Prune, Cover a planting area, Weed a planting area, Till a planting area, Tidy a workspace, Apply fertilizer, Apply water, Custom), completing it is a plain toggle - there's no Plant data to write.
