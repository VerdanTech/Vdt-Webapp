...
from src.domain.user import User, commands, events
from src.interfaces.persistence.common import AbstractUow
from src.interfaces.security import AbstractPasswordCrypt
from src.ops.exceptions import EntityNotFound, IncorrectPassword
from src.ops.common import CommandHandlerDict

async def create_user(command: commands.CreateUser, svcs_container: Container) -> None:
    """
...
async def update_user(
    command: commands.UpdateUser, svcs_container: Container, client: User,
) -> None:
    """
    ...
    async with uow:
        ...
        user.events.append(
            events.EmailPendingConfirmation(
                username=user.username, email=command.email_address
            )
        )
...
async def confirm_email_confirmation(
    ...
    async with uow:
        # Retrieve user from persistence
        user = await uow.users.get_user_by_email_confirmation_key(
...
async def confirm_password_reset(
    ...
    async with uow:
        # Retrieve user from persistence
        user = await uow.users.get_user_by_password_reset_confirmation(
            user_id=command.user_id, email_confirmation_key=command.key
        )
        if user is None:
        ...
        await user.password_reset_confirm(
            ...
            key=command.key,
            new_password=command.new_password1,
            password_crypt=password_crypt,
        )
...
USER_ASGI_COMMAND_HANDLERS: CommandHandlerDict = {
    commands.CreateUser: create_user,
    commands.UpdateUser: update_user,
    commands.RequestEmailConfirmation: request_email_confirmation,
    commands.ConfirmEmailConfirmation: confirm_email_confirmation,
    commands.RequestPasswordReset: request_password_reset,
    commands.ConfirmPasswordReset: confirm_password_reset,
}
