...
from src.common.ops.messages import MessageProcessor
...
class UserCommandController(Controller):
    ...
    async def user_login(
        self, data: commands.Login, state: State, processor: MessageProcessor
    ) -> Response[None]:
        """
        ...
        """
        processor.svcs_container.register_local_value(State, state)
        uow, password_crypt = await processor.svcs_container.aget_abstract(
    ...
    async def user_email_confirmation_request(
        ...
        processor.svcs_container.register_local_value(State, state)
        with litestar_exception_map():
            await processor.handle(message=data)
