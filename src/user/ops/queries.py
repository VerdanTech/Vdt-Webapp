...
class ClientProfileQuery(Query):
    """
    Retrieves the client's profile.

    Blank as the client user is supplied seperately,
    but a Query object in the handler signature is required
    to confirm to the QueryHandler type.
    """
...
async def verify_password(
    ...
    async with uow:
        user = await uow.repos.users.get_by_email_addresses(query.email_address)
        if user is None:
...
async def client_profile(
    query: ClientProfileQuery, svcs_container, client: User | None = None
) -> UserFullSchema:
    """
    Returns the full profile of the client user.

    Args:
        query (None): the query object.
        svcs_container (Container): service locator.
        client (User | None): the client user.
