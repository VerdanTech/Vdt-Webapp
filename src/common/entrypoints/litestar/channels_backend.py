...
class RedictChannelsBackend(ChannelsBackend):
    """An litestar channels backend to connect to the Redict server."""

    def __init__(self, history: int = 0) -> None:
        self._max_history_length = history
        self._channels: set[str] = set()
        self._queue: Queue[tuple[str, bytes]] | None = None
        self._history: defaultdict[str, deque[bytes]] = defaultdict(
            lambda: deque(maxlen=self._max_history_length)
        )
    ...
    async def publish(self, data: bytes, channels: Iterable[str]) -> None:
        ...
        if not self._queue:
            raise RuntimeError(
                "Backend not yet initialized. Did you forget to call on_startup?"
            )
