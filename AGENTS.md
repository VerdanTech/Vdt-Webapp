...
- **The Planner canvas** (`packages/ui/src/components/canvas`) renders with native SVG driven directly through Svelte reactivity (bound attributes, `{#if}`/`{#key}`, pointer-event handlers) - `Geometry` is limited to four parametric shapes. Resize handles are SVG `<circle>` elements nested in the same `<g>` as their shape, inheriting its rotation/position for free. This lets the planned SVG-based Cultivar `IconPack` rendering nest an `IconPackVariant`'s SVG directly as a child element instead of needing a raster-conversion step, and makes dragging a Plant between two simultaneously-displayed Workspaces (the main motivating case for ever rendering more than one Workspace in the Layout at once, e.g. to transplant by dragging from a seed-starting area into a bed) tractable: real DOM nodes let each Workspace be its own positioned `<g>`/nested `<svg>`, so detecting a drag crossing from one into the other is ordinary DOM geometry - that multi-workspace-pane feature itself is not yet built. If large-farm scenes ever make SVG node count a real bottleneck, the plan is viewport culling and level-of-detail rendering (mirroring the viewport-clipped approach `grid.svelte.ts` already uses for gridlines) - not in progress, a deliberate future step if the need arises.
...
- No special Unicode arrows (→, ⇒, etc.) or other non-ASCII punctuation stand-ins anywhere the em dash rule above applies. Use `->` instead of `→`, and otherwise prefer the plain ASCII punctuation a standard keyboard types.
