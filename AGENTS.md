...
- **The Planner canvas** (`packages/ui/src/components/canvas`) currently renders with Konva (vanilla, not `svelte-konva`), with components manually syncing Svelte reactive state into Konva's imperative scene graph. The intention is to migrate to native SVG rendered directly through Svelte reactivity instead: the app's actual usage of Konva doesn't touch its differentiating features (no `Transformer`, image filters, caching, or pixel hit-testing - `Geometry` is limited to four parametric shapes, and resize handles are already a hand-rolled DOM overlay, not Konva's), and going SVG-native removes the imperative sync layer entirely while aligning with the planned SVG-based Cultivar `IconPack` rendering. It would also make dragging a Plant between two simultaneously-displayed Workspaces (the main motivating case for ever rendering more than one Workspace in the Layout at once, e.g. to transplant by dragging from a seed-starting area into a bed) more tractable: real DOM nodes let each Workspace be its own positioned `<g>`/nested `<svg>`, so detecting a drag crossing from one into the other is ordinary DOM geometry, rather than manually converting pointer coordinates between separate Konva Stage coordinate systems. If large-farm scenes ever make SVG node count a real bottleneck, the plan is viewport culling and level-of-detail rendering (mirroring the viewport-clipped approach `grid.svelte.ts` already uses for gridlines) before reaching for a canvas library again. This isn't in progress; it's a deliberate future step.
