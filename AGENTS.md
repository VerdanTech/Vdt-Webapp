...
- **The Planner canvas** (`packages/ui/src/components/canvas`) renders with native SVG driven directly through Svelte reactivity (bound attributes, `{#if}`/`{#key}`, pointer-event handlers), migrated off Konva since the app's actual usage never touched Konva's differentiating features (no `Transformer`, image filters, caching, or pixel hit-testing - `Geometry` is limited to four parametric shapes). Resize handles are SVG `<circle>` elements nested in the same `<g>` as their shape, inheriting its rotation/position for free, the same way they inherited it as nested `Konva.Circle` shapes before the migration. Going SVG-native removed the imperative sync layer entirely (no more manually pushing Svelte state onto an imperative scene graph) and enables the planned SVG-based Cultivar `IconPack` rendering: an `IconPackVariant`'s SVG can be nested directly as a child element instead of needing a raster-conversion step. It also makes dragging a Plant between two simultaneously-displayed Workspaces (the main motivating case for ever rendering more than one Workspace in the Layout at once, e.g. to transplant by dragging from a seed-starting area into a bed) more tractable: real DOM nodes let each Workspace be its own positioned `<g>`/nested `<svg>`, so detecting a drag crossing from one into the other is ordinary DOM geometry, rather than manually converting pointer coordinates between separate Konva Stage coordinate systems - that multi-workspace-pane feature itself is not yet built. If large-farm scenes ever make SVG node count a real bottleneck, the plan is viewport culling and level-of-detail rendering (mirroring the viewport-clipped approach `grid.svelte.ts` already uses for gridlines) before reaching for a canvas library again - not in progress, a deliberate future step if the need arises.
...
| Canvas         | Native SVG (Svelte-driven)                      |
...
- No em dashes (—) anywhere: code comments, design docs, commit messages, PR descriptions, chat responses. Use a regular hyphen (-) instead.
